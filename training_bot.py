...
async def handle_messages(message: Message):
    try:
        # Проверяем, запрашивает ли пользователь изображение
        if "нарисуй" in message.text.lower() or "покажи картинку" in message.text.lower():
            # Получаем промпт для изображения от GPT
            image_prompt = get_chatgpt_response(f"Создай детальный промпт на английском языке для генерации изображения по запросу пользователя: {message.text}")
            ...
            if image_url:
                ...
                await message.answer("Вот изображение, которое я создал по вашему запросу! Что вы о нём думаете?")
            else:
                await message.answer("Извините, не удалось создать изображение. Давайте попробуем что-то другое?")
            return
